...
Mutable interpretation state no longer lives in `model.Assumptions`. Shared semantics uses an `OperationContext` containing the immutable `world: Assumptions`, `VariableBindingsState`, and semantically passive resolver observer. Resolver26 extends that boundary with its request scope and explicit `cycleChecker: CycleCheckState`, `bindingDeclarationsState: BindingDeclarationsState`, `queryValuesState: QueryValuesState`, and `objectOrchestrationState: ObjectOrchestrationState` properties; the operation context does not implement those state protocols. `OEROccurrenceContext` carries the stable root, exact path, target, and optional structural parent bundle through Resolver26 object orchestration and field-resolution tasks.
...
Resolver02/03, Resolver07/08, Resolver22/23, and Resolver26 support `@parent` fields. Schema preparation validates an argument-free singular parent field against its unique argument-free child producer and retains the parent-to-child transpose; child producers may return singular objects, lists, or nested lists. Each child parent cell points to its actual containing ancestor OER, but EOD materialization creates a fresh finite parent projection rather than preserving resolver-visible object identity. One-level demand lifting composes for grandparent selections. Conservative lifting may over-resolve or over-materialize ancestor demand when an active grandchild field is dynamically passive; this is an accepted tradeoff for the uncommon overlap of those features.

Resolver26 keeps a request-local OER-to-orchestration registry and accepts incremental ancestor-demand waves discovered after child occurrence construction. It combines static parent-demand transposition for selective producer output with dynamic exact and symbolic lifting from child orchestration. Potential child producers remain mutable until request quiescence so late lifted demand can install cells; non-producers preserve eager freezing.

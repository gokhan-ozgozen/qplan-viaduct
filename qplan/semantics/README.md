...
Resolver02/03, Resolver07/08, Resolver22/23, and Resolver26 resolve `@parent` selections. Each installs the child's parent cell with the containing ancestor OER itself, including for list and nested-list child occurrences. Demand on `parent { ... }` is lifted one ancestor at a time, so repeated closure handles grandparents. Resolver26 combines conservative static lifting into selective producer demand with exact dynamic lifting once symbolic child occurrence identity exists.
